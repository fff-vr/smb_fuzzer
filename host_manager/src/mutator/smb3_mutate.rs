...
pub fn smb3_mutate_dumb(data: &mut Vec<u8>, mutation_rate: f32)->input_queue::Fragments{
    ...
    for _ in 0..num_mutations {
        ...
        data[index] = value;
        let corpus = input_queue::Fragment{offset:index,value : value};
...
fn apply_fragments(data : &mut Vec<u8>, fragments : &input_queue::Fragments){
    for fragment in fragments.iter(){
        if fragment.offset >= data.len(){
            //TODO move to debug
            println!("[W] fragment.offset >= data.len() ... {} >= {}",fragment.offset,data.len());
            return;
        }
        data[fragment.offset] = fragment.value;
    }
}
pub fn smb3_mutate_coverage(data: &mut Vec<u8>, mutation_rate: f32, fragments : input_queue::Fragments)->input_queue::Fragments{
